...
PythonのCGIサーバを使ってローカルで動かすことができます。

```bash
bash start.sh
```

difff のディレクトリに移動しなくても、フルパスで指定すれば任意の場所から実行できます。

```bash
bash /path/to/difff/start.sh
```
...
> **注意：** `index.cgi` は起動時に `server.py` が `difff.pl` から自動生成します。
